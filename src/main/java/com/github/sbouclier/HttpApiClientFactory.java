...
public class HttpApiClientFactory {
    ...
    public HttpApiClient<? extends Result> getHttpApiClient(String apiKey, String apiSecret, KrakenApiMethod method) {
        switch (method) {
            ...
            case WITHDRAW:
                return new HttpApiClient<WithdrawResult>(apiKey, apiSecret);
            case ADD_ORDER:
                return new HttpApiClient<AddOrderResult>(apiKey, apiSecret);
            default:
                throw new IllegalArgumentException("Unknown Kraken API method: " + method);
        }
    }
}
